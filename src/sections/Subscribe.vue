<template>
    <section id="contact-us" class="max-container flex justify-between items-center max-lg:flex-col gap-10 reduce-bottom-space">
      <h3 class="text-4xl leading-[68px] lg:max-w-md font-palanquin font-bold">
        Sign Up for
        <span class="text-red-600"> Updates </span>& Newsletter
      </h3>
      <div class="lg:max-w-[40%] w-full flex items-center max-sm:flex-col gap-5 p-2.5 sm:border sm:border-slate-gray rounded-full">
        <input type="text" placeholder="subscribe@zionet.com" class="input" />
        <div class="flex max-sm:justify-end items-center max-sm:w-full">
          <Button label="Sign Up" :iconUrl="shieldTick" fullWidth />
        </div>
      </div>
    </section>
</template>

<script setup>
import Button from '../components/Button.vue';
import {shieldTick} from "../assets/icons";
</script>